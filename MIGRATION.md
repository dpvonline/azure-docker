...
Infrastruktur steht, Postgres 18 läuft auf `/data/postgres` mit allen drei Datenbanken,
WAL-Archivierung und Full-Backup gehen durch. Offen sind nur noch die Absicherungen aus
dem Vorfall unten (eigener PR, danach einmal VM-Neuaufbau als Test des neuen
Boot-Pfads) und das Einspielen von `keycloak.dump`.

## Vorfall August 2026: OS-Platte voll, alles still ausgefallen

**Was passiert ist.** Ab dem 12.08. war die OS-Platte der VM voll. Daraufhin lief
Postgres fünf Wochen in einer Restart-Schleife, der Azure Monitor Agent konnte sich
nicht mehr aktivieren, Azure Backup scheiterte an jedem einzelnen Tag, und der
Sonntags-Update-Lauf brach jede Woche ab. Aufgefallen ist es erst am 23.09., als ein
`az vm run-command` nicht zurückkam. Produktivdaten waren nicht betroffen, auf der VM
lagen nur Testdaten.

**Ursachenkette.**

1. Das `apply` im Juli lief vom Branch `phase-0-fundament`, PR #20 war aber noch nicht
   gemergt. `custom_data` kam also vom Branch, die VM klonte beim Boot jedoch `main`.
   Dort zeigte die Compose-Datei noch auf `/mnt/postgres-data`, was auf dieser
   VM-Größe ohne Temp-Disk einfach ein Verzeichnis auf der OS-Platte ist. Docker legte
   es stillschweigend an.
2. Die Postgres-Platte war beim ersten Boot noch nicht angehängt, weil sie im selben
   `apply` ersetzt wurde. cloud-init wartete 60 Sekunden, gab auf und versuchte es nie
   wieder. `fstab` hat `nofail`, also blockierte auch nichts den Start.
3. Der pgBackRest-Stanza-Reset (Schritt 5 der Verifikation oben) wurde nie ausgeführt.
   Das neue Cluster hatte eine neue System-ID, und **jede** WAL-Archivierung scheiterte
   am Mismatch. Postgres hebt nicht archivierte Segmente auf: mit
   `archive_timeout=600` rund 2 GB am Tag, zusammen 2.512 Segmente oder 40 GB.
4. Die Verifikationsliste oben hätte alle drei Ursachen gefunden. Sie wurde nach dem
   `apply` nicht abgearbeitet.

**Warum niemand etwas gemerkt hat.** Jede einzelne Stelle hat den Fehler erkannt, aber
keine hat ihn an jemanden gemeldet:

| Wer wusste es | Was fehlte |
|---|---|
| Azure Backup: 30 Alarme „Backup Failure", täglich | Vault nicht mit der Action Group verbunden |
| `dpv-update`: jeden Sonntag `ABORT` | stand nur in `/var/log/dpv-update.log` |
| Nächtliches pgBackRest-Backup | stand nur in `/var/log/pgbackrest-full.log` |
| Platten-Alert | Agent tot, also keine Daten — und keine Daten sieht aus wie „alles gut" |

**Behebung am 23.09.** `/mnt/postgres-data` gelöscht, die Postgres-Platte gemountet,
`main` auf der VM gezogen, Cluster neu initialisiert, Stanza zurückgesetzt, Full-Backup
erfolgreich, Monitor-Agent reaktiviert (Perf-Daten kamen nach drei Minuten), Azure
Backup von Hand angestoßen.

**Absicherungen dagegen** (PR „Stille Ausfälle abfangen"):

- `scripts/mount-data-disks.sh` mountet die Platten bei **jedem** Start und verweigert
  den Stack, solange eine fehlt; `dpv-compose.service` versucht es alle zwei Minuten
  erneut.
- `create_host_path: false` in der Compose-Datei: fehlt das Quellverzeichnis, startet
  Postgres nicht, statt es auf der OS-Platte anzulegen.
- `boot.sh` und die Unit liegen jetzt im Repo statt in `custom_data`, damit solche
  Korrekturen ohne VM-Neuaufbau auf die VM kommen.
- Alle drei Skripte melden Fehler per Syslog, ein Alert wertet das aus.
- Die Backup-Alarme des Vaults gehen an die Action Group.
- Ein Totmannschalter löst aus, wenn der Monitor-Agent keine Daten mehr liefert.
- Log Analytics hält 90 statt 30 Tage — nach 30 Tagen waren die Daten vom 12.08.
  schon weg und ließen sich nicht mehr rekonstruieren.
- README: erst mergen, dann `apply`.
